 ...
 */
@IonicPage({ segment: 'addon-resource-my-resource' })
@Component({
    selector: 'page-addon-resource-my-resource',
    templateUrl: 'my-resource.html',
})
export class AddonResourceMyResourcePage implements OnDestroy {
    ...
    protected fetchUserCourses(): Promise<any> {
        return this.coursesProvider.getUserCourses(null, null, 2).then((courses) => {
            const promises = [],
                courseIds = courses.map((course) => {
                return course.id;
            });
